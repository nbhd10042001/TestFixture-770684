...
class SerialGUI:
    ...
    def __init__(self, master):
        self.master = master
        master.title("Test Fixture For 770684")
        ...
        bottom_left_frame = tb.Frame(left_frame)
        bottom_left_frame.pack(anchor='w', padx=10, pady=10)

        tb.Label(bottom_left_frame, font=("Arial", 8, "italic"), text="Product of Saigon Fab-9 / Version 1.0").grid(row=0, column=0, sticky='w')
        tb.Label(bottom_left_frame, font=("Arial", 8, "italic"), text="Author: Duc Nguyen").grid(row=1, column=0, sticky='w')

        # ===== Right Frame =====
        right_frame = tb.Frame(center_frame)
        right_frame.grid(row=0, column=1, sticky="nsw", padx=10)
